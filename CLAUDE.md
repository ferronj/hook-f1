...
- `add_agent_prediction.py` — Helper to merge an agent-authored forecast into a sim JSON (invoked only when Claude is asked to predict; never called by the simulation script itself)
- `dashboard.py` — Multi-race Streamlit dashboard, auto-discovers `data/sim_*.json` files; renders `agent_prediction` if present

### Models (`models/`)
Production models used by the prediction pipeline are Stage 6 and Stage 9. Stage 3 serves as the shared utility hub.
...
- `stage8_plackett_luce.py` — Time-varying Plackett-Luce. **Retained as a Stage 9 dependency only** (provides PL utilities). No longer in the prediction pipeline — was consistently degenerate (P(win) > 50% on a single driver) and auto-excluded from the composite.
...
- `sim_*.json` — Simulation output files consumed by the dashboard (e.g. `sim_2026_australia.json`, `sim_2026_japan.json`)
...
stage3_constructor.py  (shared: F1DataLoader, constants, prepare_transitions, count matrices)
  ...
  ├── stage8_plackett_luce.py        (PL utilities; retained as Stage 9 dependency only)
...
```
`simulate_race.py` adds `models/` to sys.path and imports from stages 6 and 9 (Stage 8 imported transitively by Stage 9 only).
`dashboard.py` has NO model imports — reads only JSON, including the optional `agent_prediction` block.
...
| Composite | Balance | — | — | Equal-weight blend of Stage 6 + Stage 9 |
| Agent (optional) | Current-form context | — | — | Subjective; absent unless Claude is asked to predict |

Stage 8 (Time-Varying Plackett-Luce) was removed from the prediction pipeline after consistently going degenerate (P(win) > 50% on a single driver). The file remains as a Stage 9 import dependency.
...
- **Dependencies**: streamlit, plotly (managed via `pyproject.toml`)
- **Data source**: `data/sim_*.json` (auto-discovered; generated by simulation scripts)
- **Views** (sidebar switcher):
  - **Race detail** (default): Model selector (Stage 6/Composite/Stage 9), podium prediction, full grid table, position distributions, model comparison, constructor analysis, position heatmap. Optional **Agent Forecast** section renders only when `agent_prediction` is present in the sim JSON — gracefully omitted otherwise. Once race results land, an **Actuals vs. Prediction** scorecard appears per race.
  - **Season summary**: Cross-race scorecard — metric selector (top-3 hits / top-10 overlap / MAE / Spearman ρ), line chart by round, per-race table, season totals; plus a next-race preview card (composite podium + agent forecast if present) for the first race whose `date >= today`.

## Agent Prediction
The simulation script never calls an LLM. When the user asks Claude to predict a race, the workflow is:
1. Run `simulate_race.py` to produce model-only predictions
2. Web-search current form (standings, weekend signals, prior-season comparable)
3. Author a prediction JSON conforming to the schema in `add_agent_prediction.py`
4. Run `uv run python add_agent_prediction.py --sim <sim.json> --prediction <agent.json>`
5. The dashboard auto-renders the agent forecast next time it loads

Running `simulate_race.py` directly (without invoking Claude) skips steps 2-4 and produces a sim file with no `agent_prediction` block — the dashboard renders without that section.
