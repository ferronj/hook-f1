...
- `simulate_2026_australia.py` — Monte Carlo simulation of 2026 Australian GP (Stage 6 + Stage 8 + Stage 9 + Composite), outputs JSON
- `simulate_2026_japan.py` — Monte Carlo simulation of 2026 Japanese GP (Stage 6 + Stage 8 + Stage 9 + Composite), outputs JSON
- `simulate_2026_race.py` — Generic race simulation script (sequential race-by-race with in-season model updates)
- `dashboard.py` — Multi-race Streamlit dashboard, auto-discovers `data/sim_*.json` files (`uv run streamlit run dashboard.py`)
...
- Dashboard: `micromamba run -n f1-markov streamlit run dashboard.py`
...
- Simulation results stored as JSON in `data/sim_*.json` (consumed by dashboard), e.g. `sim_2026_australia.json`, `sim_2026_japan.json`

## Dashboard
- **Streamlit app**: `dashboard.py`
- **Run**: `uv run streamlit run dashboard.py`
- **Dependencies**: streamlit, plotly (managed via `pyproject.toml`)
- **Data source**: `data/sim_*.json` (auto-discovered; generated by `simulate_2026_*.py` scripts)
- **Features**: Model selector (Stage 6/Composite/Stage 9), podium prediction, full grid table, position distributions, model comparison, 2025 calibration check, constructor analysis, position heatmap
